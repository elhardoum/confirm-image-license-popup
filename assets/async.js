($ =>
{
  ...
  confirmDialog = _ => new Promise((resolve, reject) =>
  {
    ...
    let rtl = document.body.classList.contains('rtl')
      , popup = $(`<div style="position:fixed;top:0;left:0;width:100%;height:100%;z-index:999999">
      ...
      <div style="position:relative;top:50%;background:white;box-shadow:0 15px 30px 0 rgba(0,0,0,.11),0 5px 15px 0 rgba(0,0,0,.08)!important;padding:1rem;width:500px;left:50%">
        ...
        <span style="position:absolute;top:0;${rtl?'left':'right'}:0;background:#ddd;padding:14px 10px;line-height:0;font-size:1rem;cursor:pointer;user-select:none">&times;</span>
      </div>
    ...
    $(window).on('resize', _ => popup && popup.children().eq(1).css({
      [rtl?'right':'left']: document.documentElement.clientWidth/2 - popup.children().eq(1).outerWidth()/2,
